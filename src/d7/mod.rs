...
fn get_hand_type1(hand: &String) -> i32 {
...
fn sum_hands(sorted_hands: Vec<(String, i32)>) -> i32 {
    let mut winnings = 0;

    for (rank, (_, bet)) in sorted_hands.iter().enumerate() {
        winnings += bet * (rank as i32 + 1);
    }

    winnings
}

fn camels1(file_path: &str) -> i32 {
...
fn get_hand_type2(hand: &String) -> i32 {
    let mut card_counts = HashMap::new();
    for card in hand.chars() {
        card_counts.entry(card).and_modify(|x| *x += 1).or_insert(1);
    }
