        ...
        for (x, char) in line.chars().enumerate() {
            ...
            if number_start_index >= 0 && (x + 1 == line.len() || !line.chars().nth(x+1).unwrap().is_numeric()) {
                ...
                for (check_x, check_y) in coord_generator(number_start_index, x as i32+1, y as i32) {
                    // If they're in range,
                    if check_x >= 0 && check_x < line.len() as i32 && check_y >= 0 && check_y < lines.len() as i32 {
                        let char_check: char = lines[check_y as usize].chars().nth(check_x as usize).unwrap();
            ...
            if number_start_index >= 0 && (x + 1 == line.len() || !line.chars().nth(x+1).unwrap().is_numeric()) {
                ...
                for (check_x, check_y) in coord_generator(number_start_index, x as i32+1, y as i32) {
                    ...
                    if check_x >= 0 && check_x < line.len() as i32 && check_y >= 0 && check_y < lines.len() as i32 {
                        ...
                        if char_check == '*' {
                            gear_candidates.entry((check_y, check_x))
